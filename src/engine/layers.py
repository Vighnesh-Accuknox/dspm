...
ADDRESS_KEYWORDS = ["street", "road", "avenue", "postal code", "zip code", "city", "state", "country"]

# Layer 2: Credentials and Secrets
PASSWORD_REGEX = re.compile(r'(?i)(password|passwd|pwd)\s*[:=]\s*[\'"]?([^\s\'"]+)[\'"]?')
...
OAUTH_TOKEN_REGEX = re.compile(r'(?i)(access_token|refresh_token|id_token)\s*[:=]\s*[\'"]?([A-Za-z0-9\-._~+/]+=*)')
...
BANK_ACCOUNT_KEYWORDS = ["account", "bank", "routing", "ifsc", "iban", "swift", "beneficiary"]

# Layer 4: Healthcare Data
HEALTHCARE_KEYWORDS = ["patient", "diagnosis", "treatment", "prescription", "hospital", "medical", "doctor", "insurance", "health record"]
...
REGIONAL_REGEXES = {
    ...
    "IN": {
        ...
        "GST": re.compile(r"\b((?:0[1-9]|[1-3][0-7])[A-Za-z0-9]{10}[A-Za-z0-9]{1}Z[A-Za-z0-9]{1})\b"),
...
CONTEXT_WORDS = {
    ...
    "Bank Account": ["account", "bank", "routing", "ifsc", "iban", "swift", "beneficiary"],
...
def calculate_match_score(text: str, start: int, end: int, base_score: float, detector_name: str) -> float:
    """
    ...
    for kw in keywords:
        if kw in context_window:
            ...
            boost = 0.35
            if detector_name in ["Phone Number", "Date of Birth", "US SSN", "IN Aadhaar", "IN PAN", "CA SIN", "GB NINO", "IN GST"]:
                boost = 0.45
...
def is_starred_private_key(text: str, start: int) -> bool:
    ...
    footer_pattern = re.compile(r'(-----END [A-Z ]+PRIVATE KEY-----|------? ?END [A-Z ]+KEY------?)')
    footer_match = footer_pattern.search(sub_text)
    if footer_match:
        # Extract content between header and footer
        header_end_match = re.search(r'(-----BEGIN [A-Z ]+PRIVATE KEY-----|------? ?BEGIN [A-Z ]+KEY------?)', sub_text)
        if header_end_match:
