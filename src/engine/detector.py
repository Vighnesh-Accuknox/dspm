...
matches into reportable findings.

    raw layers  ->  field-name suppression  ->  score threshold  ->  overlap
    resolution (one finding per span, most specific detector wins)  ->  value
    masking + fingerprint
...
Configuration keys (all optional):
    ...
    score_threshold               minimum score to report (default 0.8)
...
def _bare_number(value: str) -> bool:
    """Digits (or 1-2 letters + digits) with no separators and at most 12 chars."""
    return re.fullmatch(r"[A-Za-z]{0,2}\d{6,12}", value) is not None
...
class DetectionEngine:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.threshold = float(self.config.get("score_threshold", 0.8))
    ...
    # ------------------------------------------------------------------
    def scan_text(self, text: str, field_name: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        context scoring and structural suppression.
        """
        if not text or not isinstance(text, str):
            return []

        findings: List[Dict[str, Any]] = []
        findings.extend(scan_pii(text, field_name, self.phone_regions))
        ...
        findings.extend(scan_regional(text, self.enabled_regions, field_name, self.threshold))
        findings.extend(scan_generic(text, field_name, self.threshold))
        findings.extend(
            scan_entropy(
        ...
        if self.decode_base64:
            findings.extend(self._scan_encoded(text, field_name))

        if self.field_suppression and field_name:
            # corroborated findings (keyword, neighbouring access key, self-describing format) are exempt
            findings = [f for f in findings if f.get("evidence") or not is_suppressed_by_field(f["detector"], field_name, f.get("value"))]
        for f in findings:
        ...
        for f in findings:
            if f["category"] in ("Regional Compliance", "Financial Data") and _placeholder_number(f["value"]):
                ...
                f["placeholder"] = True
        findings = [f for f in findings if f.get("score", 0.0) > self.threshold]
    ...
    def _apply_engine_policies(self, f: Dict[str, Any]) -> None:
        ...
        Engine-level judgement on top of the recognizer semantics:
          * a checksum alone is weak evidence for a bare number - mod-10/mod-11
            passes ~10% of random ids and timestamps, so a bare, separator-free
            match needs a context word or a field hint to be reported;
          * epoch timestamps are never identifiers;
          * private / loopback / link-local IPs are infrastructure, not PII.
        """
        value = str(f.get("value", ""))
        if f.get("pattern") is not None and not f.get("context_word") and not f.get("field_hint") and _bare_number(value):
            f["score"] = min(f["score"], 0.75)
            f["needs_context"] = True
        if f["category"] in ("Regional Compliance", "Financial Data", "Healthcare Data (PHI)") and _looks_like_epoch(value):
            f["score"] = min(f["score"], 0.3)
            f["placeholder"] = "epoch"
    ...
    def _scan_encoded(self, text: str, field_name: Optional[str]) -> List[Dict[str, Any]]:
        """
        ...
        for match in self._B64_BLOB_RE.finditer(text):
            ...
            inner.extend(scan_pii(decoded, None, self.phone_regions))
            for f in inner:
